...
@pytest.mark.asyncio
async def test_output_schema_validated_once_across_agent_and_workflow_node(
    mocker,
):
  """Verifies output_schema is not re-validated when set_model_response or output_key already validated it."""
  ...
  invocation_context = await _create_invocation_context(agent)
  ctx = Context(invocation_context)

  validate_spy = mocker.spy(
      PersonSchema,
      'model_validate_json',
  )

  final_event = create_final_model_response_event(
      invocation_context,
      '{"name": "Alice", "age": 30, "city": "NYC"}',
      validated_response={'name': 'Alice', 'age': 30, 'city': 'NYC'},
  )
  agent._LlmAgent__maybe_save_output_to_state(final_event)
  process_llm_agent_output(agent, ctx, final_event)

  assert final_event.actions.state_delta['person'] == {
      'name': 'Alice',
      'age': 30,
      'city': 'NYC',
  }
  assert final_event.output == {'name': 'Alice', 'age': 30, 'city': 'NYC'}
